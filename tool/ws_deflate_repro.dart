...
Future<int> _loadIsolate((int port, int perConn, int millis, bool deflate) a) async {
...
Future<void> main() async {
  ...
  final server = await NitroServer.bind(ServerConfig(
      port: 0, isolates: 1, maxRequestsPerConnection: ka ? 0 : 100));
  ...
  stdout.writeln('[$mode] SEQ_MS=$seqMs ISO=$iso CONN=$conn LOAD_MS=$loadMs '
  ...
  final loads = [for (var i = 0; i < iso; i++) _spawn(port, conn, loadMs, deflate)];
